...
for index_organoid in tqdm(inds_organoids):
    ...
    data = tifffile.imread(f"{path_to_data}/data/ag{index_organoid}_norm.tif")
    mask = tifffile.imread(f"{path_to_data}/mask/ag{index_organoid}_mask.tif")

    sum_sigmas = sum(sigmas)
    im = np.zeros(
        (4 * sum_sigmas + len(sigmas) + 10 * (len(sigmas) - 1), 4 * max(sigmas) + 1)
    )
    print(im.shape)
    for ind, (sigma, gamma) in enumerate(zip(sigmas, [1, 1, 1])):
        for i in range(4 * sigma + 1):
            for j in range(4 * sigma + 1):
                im[
                    i
                    + np.cumsum([0] + (4 * np.array(sigmas) + 1).tolist())[ind]
                    + 10 * ind,
                    j,
                ] = np.exp(
                    -((i - 2 * sigma) ** 2 + (j - 2 * sigma) ** 2) / (2 * sigma**2)
                )
        ...
        if sigma == sigmas[0]:
            ...
            print(
                f"index organoid {index_organoid} percs_density {percs_density*(10/0.621)**3}"
            )
            print(volume_fraction[mask].min(), volume_fraction[mask].max())
            percs_volume_fraction = np.percentile(volume_fraction[mask], [1, 99])
            print(
                f"index organoid {index_organoid} percs_volume_fraction {percs_volume_fraction}"
            )
            percs_nuclear_volume = np.percentile(nuclear_volume[mask], [1, 99])
            print(
                f"index organoid {index_organoid} percs_nuclear_volume {percs_nuclear_volume * 0.621**3}"
            )
        ...
        imrgb = np.clip(
            (imrgb - percs_density[0]) / (percs_density[1] - percs_density[0]),
            0.001,
            0.999,
        )
        imrgb[mask.copy() == 0] = 0
        ...
        imrgb = np.clip(
            (imrgb - percs_volume_fraction[0])
            / (percs_volume_fraction[1] - percs_volume_fraction[0]),
            0.001,
            0.999,
        )
        imrgb[mask.copy() == 0] = 0
        ...
        imrgb = np.clip(
            (imrgb - percs_nuclear_volume[0])
            / (percs_nuclear_volume[1] - percs_nuclear_volume[0]),
            0.001,
            0.999,
        )
        imrgb[mask.copy() == 0] = 0
    ...
    napari_true_strain[:, 1] = (
        napari_true_strain[:, 1] / np.median(lengths[:, None]) * 5
    )
    ...
    napari_true_strain_resampled[:, 1] = (
        napari_true_strain_resampled[:, 1] / np.median(lengths[:, None]) * 5
    )
    ...
    viewer3.add_vectors(
        ...
        vector_style="line",
        blending="opaque",
        edge_colormap=cmap_vectors_nematic,
        length=1.5,
        edge_width=5,
    )
    viewer3.add_points(ndim=2)
    viewer3.add_vectors(
        napari_true_strain_resampled,
        name="true_strain_resampled",
    ...
    viewer3.add_points(ndim=2)
    viewer3.add_image(
        data, name="data", colormap="gray_r", contrast_limits=[0.14, 0.5], opacity=0.5
    )
    viewer3.add_vectors(
        napari_true_strain_resampled,
        name="true_strain_resampled",
    ...
    viewer4.add_image(
        dot_product_map, name="dot_product_map", colormap="turbo", opacity=0.5
    )
