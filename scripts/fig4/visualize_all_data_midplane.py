...
def get_napari_angles_cmap(colors=None, color_mode="recirculation"):
    ...
    if colors is None:
        ...
        cmap = cmap_class(colors_strs, controls=np.linspace(0, 1, len(colors_strs)))
    else:
...
for index_organoid in tqdm(indices_to_display):
    ...
    for i, sigma in enumerate([10, 20, 40]):
        ...
        viewer.add_vectors(
            true_strain_tensor_resampled,
            name=f"true_strain_tensor_resampled_{sigma}",
            vector_style="line",
            blending="opaque",
            edge_colormap=cmap,
            properties={"angles": true_strain_tensor_resampled_angles},
            length=tstrs[sigma],
        )
