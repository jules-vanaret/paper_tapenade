...
def smooth_labels_values2(labels, values, sigma, mask, mask_for_volume):
    ...
    smoothed_array = gaussian_smooth_dense(
        ...
        sigmas=sigma,
        mask=mask,
        mask_for_volume=array_centroids.astype(bool),
    )
...
vector_lenghts_true_strain_2d = np.linalg.norm(
    napari_vectors_true_strain_2d[:, 1], axis=1
)
...
viewer.add_image(
    data * 0, opacity=0.5, colormap="gray_r", contrast_limits=(0, 0.6), visible=False
)
...
viewer.add_vectors(
    ...
    vector_style="line",
    edge_colormap=cmap,
    properties={"angles": angles_vectors_inertia},
)
...
viewer.add_vectors(
    ...
    blending="opaque",
    edge_colormap=cmap,
    properties={"angles": angles_vectors_inertia_2d},
)
...
viewer.add_vectors(
    ...
    vector_style="line",
    edge_colormap=cmap,
    properties={"angles": angles_vectors_true_strain},
)
...
viewer.add_vectors(
    ...
    blending="opaque",
    edge_colormap=cmap,
    properties={"angles": angles_vectors_true_strain_2d},
)
# viewer.add_labels(np.zeros(data.shape, dtype=np.uint8))
...
if do_sparse:
    ...
    smoothed_volumes = smooth_labels_values2(
        labels, volumes, sigma, mask=mask, mask_for_volume=labels.astype(bool)
    )
