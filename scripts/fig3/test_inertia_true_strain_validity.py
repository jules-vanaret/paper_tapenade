...
for index_label, prop in enumerate(props):
    ...
    print(np.sqrt(axis_decoupling_matrix @ np.linalg.eigh(prop.tensor_inertia * 5 / prop.area)[0]))
...
def tensors_to_napari_vectors(sparse_tensors, is_inertia_tensor: bool, 
...
napari_vectors, angles = tensors_to_napari_vectors(sparse_inertia_tensor, is_inertia_tensor=True, volumes=volumes, return_angles=True)
napari_vectors_true_strain, angles_true_strain = tensors_to_napari_vectors(sparse_true_strain_tensor, is_inertia_tensor=False, volumes=volumes, return_angles=True)
