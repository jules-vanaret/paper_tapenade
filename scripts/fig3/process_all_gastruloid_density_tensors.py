    ...
    for sigma in sigmas:
        ...
        if compute_deformation:
            sparse_data_deformation_smoothed = gaussian_smooth_sparse(
                sparse_data_deformation, sigmas=sigma, dim_space=3, is_temporal=False,
                n_job=-1
            )

            deformation_data = np.zeros_like(mask, dtype=np.float32)
            for i, prop in enumerate(props):
                deformation_data[prop.slice][prop.image] = sparse_data_deformation_smoothed[i, 3]
        ...
        if compute_inertia_tensor or compute_true_strain_tensor:
            ...
            smoothed_inertia_tensors_data_midplane = smoothed_inertia_tensors_data.copy()
            zs = smoothed_inertia_tensors_data_midplane[:,0]
            zs_mask = np.abs(zs - mid_plane_ind) < 5
            smoothed_inertia_tensors_data_midplane = smoothed_inertia_tensors_data_midplane[zs_mask]

            napari_vectors_inertia_midplane, angles_vectors_inertia_midplane = napari_vectors_from_tensors(
            ...
            smoothed_inertia_tensors_data_2d_midplane = smoothed_inertia_tensors_data_2d.copy()
            zs = smoothed_inertia_tensors_data_2d_midplane[:,0]
            zs_mask = np.abs(zs - mid_plane_ind) < 5
            smoothed_inertia_tensors_data_2d_midplane = smoothed_inertia_tensors_data_2d_midplane[zs_mask]

            napari_vectors_inertia_2d_midplane, angles_vectors_inertia_2d_midplane = napari_vectors_from_tensors(
