...
viewer.add_vectors(
    vectors, edge_width=2, length=3, out_of_slice_display=True, opacity=1
)

# napari.run()
### Average on labels positions
positions = (
    np.mgrid[[slice(0, dim, 20) for dim in labels.shape]].reshape(labels.ndim, -1).T
)
...
napari_vectors_inertia[:, 1] = principal_vectors[:, 0] * principal_lengths[
    :, 0
].reshape(-1, 1)
