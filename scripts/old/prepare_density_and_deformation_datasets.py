...
for index_organoid in tqdm(range(1, 9)):
    ...
    for sigma in [5, 15, 45]:
        ...
        density = gaussian_smooth_dense(
            centroids_data, sigmas=sigma, mask=mask, dim_space=3, is_temporal=False
        )

        volume_fraction = gaussian_smooth_dense(
            volume_data, sigmas=sigma, mask=mask, dim_space=3, is_temporal=False
        )
        ...
        tifffile.imwrite(
            f"{path_to_data}/cell_density/ag{index_organoid}_sigma{sigma}.tif", density
        )
