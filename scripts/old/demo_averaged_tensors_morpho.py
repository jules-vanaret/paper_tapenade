...
def napari_vectors_from_tensors(smoothed_tensors, apply_decoupling,
                                from_2D=False, positions_from_2D=None):
...
viewer.add_vectors(napari_vectors_inertia, edge_color='blue', edge_width=2, length=0.1,
                   out_of_slice_display=True, opacity=1,properties={'angles': np.arctan2(*(napari_vectors_inertia[:,1,1:].T))},
                   edge_colormap=cmap)

viewer.add_vectors(napari_vectors_inertia_2d, edge_color='blue', edge_width=2, length=0.1,
                     out_of_slice_display=True, opacity=1,properties={'angles': np.arctan2(*(napari_vectors_inertia_2d[:,1,1:].T))},
                     edge_colormap=cmap)

viewer.add_vectors(napari_vectors_true_strain, edge_color='green', edge_width=2, length=150,
                   out_of_slice_display=True, opacity=1,properties={'angles': np.arctan2(*(napari_vectors_true_strain[:,1,1:].T))},
                   edge_colormap=cmap)

viewer.add_vectors(napari_vectors_true_strain_2d, edge_color='green', edge_width=2, length=150,
                        out_of_slice_display=True, opacity=1,properties={'angles': np.arctan2(*(napari_vectors_true_strain_2d[:,1,1:].T))},
                        edge_colormap=cmap)
