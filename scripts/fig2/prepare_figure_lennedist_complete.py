...
for name in ['alice', 'mine']:
    for name_data, name_labels in zip(data_dict['raw'][name][::2], data_dict['raw'][name][1::2]):

        for modality in ['raw', 'isoscaled_norm']:

            print(name_data)
            data = tifffile.imread(
                f'{path_to_data}/{modality}/{name}/{name_data}'
            )
            print(data.shape)
            labels = tifffile.imread(
                f'{path_to_data}/{modality}/{name}/{name_labels}'
            )
            if data.ndim == 4:
                data = data[14]
                labels = labels[14]
...
for name in ['alice', 'mine']:
    for modality in ['new_datasets']:
        for name_data, name_labels in zip(data_dict[modality][name][::2], data_dict[modality][name][1::2]):
