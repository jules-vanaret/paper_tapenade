...
if not os.path.exists(path_to_csv_one):
    ...
    one_view_labels_norm = tifffile.imread(
        f"{path_to_data}/one_cropped_norm_labels.tif"
    )
    ...
    two_views_labels_norm = tifffile.imread(
        f"{path_to_data}/two_cropped_norm_labels.tif"
    )

    # intensity profiles
    intensity_one = [
        np.median(one_view_z[one_view_mask_z])
        for one_view_z, one_view_mask_z in zip(one_view, one_view_mask)
    ]
    intensity_one_norm = [
        np.median(one_view_norm_z[one_view_mask_z])
        for one_view_norm_z, one_view_mask_z in zip(one_view_norm, one_view_mask)
    ]
    intensity_two = [
        np.median(two_views_z[two_views_mask_z])
        for two_views_z, two_views_mask_z in zip(two_views, two_views_mask)
    ]
    intensity_two_norm = [
        np.median(two_views_norm_z[two_views_mask_z])
        for two_views_norm_z, two_views_mask_z in zip(two_views_norm, two_views_mask)
    ]
...
ax2.plot(
    uniform_filter(intensity_one_norm, size=3),
    label="one norm",
    c="royalblue",
    linestyle="--",
)
...
ax2.plot(
    uniform_filter(intensity_two_norm, size=3),
    label="two norm",
    c="orange",
    linestyle="--",
)
...
ax2.plot(
    uniform_filter(nlabels_one_norm, size=10),
    label="one norm",
    c="royalblue",
    linestyle="--",
)
...
ax2.plot(
    uniform_filter(nlabels_two_norm, size=10),
    label="two norm",
    c="orange",
    linestyle="--",
)
...
ax2.plot(
    uniform_filter(density_one_norm, size=10),
    label="one norm",
    c="royalblue",
    linestyle="--",
)
...
ax2.plot(
    uniform_filter(density_two_norm, size=10),
    label="two norm",
    c="orange",
    linestyle="--",
)
