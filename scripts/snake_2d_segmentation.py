...
labels_layer = viewer.add_labels(
    np.zeros(image_layer.data.shape, dtype=int),
    name='blobs labels')

shapes_layer = viewer.add_shapes(shape_type=['ellipse'], face_color=np.array([1,1,1,0])
)
...
def profile_lines_drag(layer, event):
    ...
    if 'Shift' in event.modifiers:
        ...
        roi_slices = tuple(
            slice(max(0, pos-size), min(layer_s, pos+size)) for pos, layer_s in zip(init_pos[1:], layer_shape[1:]) 
        )
        roi_slices = (slice(init_pos[0], init_pos[0]+1), *roi_slices)

        center_pos = np.array(tuple(
            size - max(size-pos, 0) for pos in init_pos[1:]
        ))
